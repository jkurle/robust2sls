# simulation variance of gauge compared to theoretical variance

gamma <- 0.05

p <- generate_param(1, 1, 1, sigma = 2, intercept = TRUE, seed = 42, Pi = matrix(c(1,0,5,2),2,2))
gauge_avar_mc(ref_dist = "normal", sign_level = gamma, initial_est = "robustified", iteration = 10, parameters = p, split = 0.5)

d <- generate_data(p, 1000)
model <- outlier_detection(data = d$data, formula = y ~ x1 + x2 | x1 + z2, sign_level = gamma, initial_est = "robustified", iterations = 2)
...
parameters <- param_gen_2SLS(dx1=dx1, dx2=dx2, dz2=dz2, sigma=sigma, seed=seed, intercept=TRUE,
                             mean_z = matrix(0,(dz-1),1), var_z = diag((dz-1)),
                             Sigma_half = diag(dx2), Omega = matrix(c(1/2,1/2),2,1),
                             beta = matrix(c(1,-3,-2,6,3),dx,1),
                             Pi = rbind(cbind(diag(dx1),matrix(0,3,2)), cbind(matrix(0,3,3),matrix(c(1,0,0,0,1,1),3,2))) )
...
if (parameters[[16]] == TRUE) { # then M_zz only dimension (dz-1) bc intercept has no var-cov
  ...
  #  Pi_sub <- Pi[2:nrow(Pi), 2:ncol(Pi)]
  #  M_xx_tilde_sub <- t(Pi_sub) %*% M_zz_sub %*% Pi_sub # is symmetric and pd

  #  M_xx_tilde_sub_inv <- inv(M_xx_tilde_sub)
  #  M_xx_tilde_inv <- cbind(matrix(0,(dx-1),1), M_xx_tilde_sub_inv)
  #  M_xx_tilde_inv <- rbind(matrix(0,1,dx), M_xx_tilde_inv)
  # comment: inverse matrices are not taken as symmetric anymore, so probably due to numeric issues
  # comment: if round inverse matrices up to 13 digits, then is symmetric -> so is likely numeric issue
...
# intermediate varrho parameters (only necessary for one formulation of avar)
rho_bb <- 2 * c * f / phi
rho_ss <- c * (c^2 - varsigma_c_2) * f / tau_c_2
rho_sb <- (c^2 - varsigma_c_2) * f * Sigma_half %*% zeta_c_minus / (2 * tau_c_2)
rho_bxu <- 1 / phi
rho_suu <- sigma / (2 * tau_c_2)
rho_sxu <- matrix(0,dx,1) # not needed, already dropped out in formula for avar

# calculate theoretical avar

# method 0: draft v6.1, p. 37, first equality
line1 <- gamma * (1 - gamma) + (c * f / sigma^2)^2 * rho_ss^2 * (tau_4-1) * sigma^4 + (2*c*f/sigma)^2 * rho_suu^2 * (tau_c_4 - tau_c_2^2/phi)
line2 <- (2*c*f/sigma)^2 * rho_ss^2 * sigma^2 * t(Omega) %*% Sigma_half %*% M_xx_tilde_inv %*% Sigma_half %*% Omega + (2*c*f/sigma)^2 * sigma^2 * t(rho_sb) %*% M_xx_tilde_inv %*% rho_sb
line3 <- (f/sigma)^2*sigma^2*rho_bb^2*t(zeta_c_minus) %*% Sigma_half %*% M_xx_tilde_inv %*% Sigma_half %*% zeta_c_minus - 2 * (2*c*f/sigma)^2*sigma^2*rho_ss*t(Omega) %*% Sigma_half %*% M_xx_tilde_inv %*% rho_sb
line4 <- -4*c*(f/sigma)^2*rho_ss*rho_bb*sigma^2*t(Omega) %*% Sigma_half %*% M_xx_tilde_inv %*% Sigma_half %*% zeta_c_minus + 4*c*(f/sigma)^2 * rho_bb * sigma^2 * t(rho_sb) %*% M_xx_tilde_inv %*% Sigma_half %*% zeta_c_minus
line5 <- (f/sigma)^2 * rho_bxu^2 * tau_c_2*sigma^2*t(zeta_c_minus) %*% Sigma_half %*% M_xx_tilde_inv %*% Sigma_half %*% zeta_c_minus - (2*c*f/sigma^2) * rho_ss * (1-gamma-tau_c_2)*sigma^2
line6 <- 4/sigma*(c*f/sigma)^2 * rho_ss * rho_suu * (tau_c_4-tau_c_2^2/phi)*sigma^2
line7 <- (-2*f/sigma*rho_bxu*tau_c_2*sigma^2*(2*c*f/sigma)*(rho_ss*t(Omega)%*%Sigma_half-t(rho_sb))) %*% M_xx_tilde_inv %*% Sigma_half %*% zeta_c_minus
line8 <- 2*(f/sigma)^2*rho_bxu*tau_c_2*sigma^2*rho_bb*t(zeta_c_minus) %*% Sigma_half %*% M_xx_tilde_inv %*% Sigma_half %*% zeta_c_minus
avar0 <- line1 + line2 + line3 + line4 + line5 + line6 + line7 + line8
...
for (m in 1:M) {
  ...
  model <- robust2SLS(data=data[[1]], regression=parameters[[12]], sign_level=gamma, iterations=1, initial_est="robustified", graph=FALSE)
  num.outliers <- sum((model[[5]][[2]] == 0))
  num.nonmissing <- nrow(data[[1]]) - sum((model[[5]][[2]] == -1))
...
print(duration)####until here new MC


test <- generate_param(dx1=dx1, dx2=dx2, dz2=dz2, sigma=sigma, seed=seed, intercept=TRUE,
                            mean_z = matrix(0,(dz-1),1), cov_z = diag((dz-1)),
                        Sigma = diag(dx2), Omega = matrix(c(1/2,1/2),2,1),
                      beta = matrix(c(1,-3,-2,6,3),dx,1),
                     Pi = rbind(cbind(diag(dx1),matrix(0,3,2)), cbind(matrix(0,3,3),matrix(c(1,0,0,0,1,1),3,2))) )

gauge_avar_mc("normal", 0.05, "robustified", 1, test, 0.5)


# more accuracy
gamma <- mpfr(0.05, 150) # significance level for selection
c <- qnorm(gamma/2, mean=0, sd=1, lower.tail = FALSE) # cut-off value corresponding to gamma
phi <- 1 - gamma # probability of non-outlying
f <- dnorm(c, mean=0, sd=1) # f(c)
tau_c_2 <- phi - 2 * c * f # variance of standardised error truncated to -c to c
...
A <- 2*c*f/phi
B <- c*(c^2-varsigma_c_2)*f/tau_c_2

vsb1 <- B
vsb2 <- A*B + B^2
vsb3 <- A^2*B + A*B^2 + B^3

identical(varrho(0.05, "normal", 1)$c$vsb, vsb1) # T
identical(varrho(0.05, "normal", 2)$c$vsb, vsb2) # T
identical(varrho(0.05, "normal", 3)$c$vsb, vsb3) # T

C <- c*(c^2-varsigma_c_2)*f/(tau_c_2*(phi-2*c*f))

vsxu1 <- ((tau_c_2-c*(c^2-varsigma_c_2)*f)/(tau_c_2-c*(c^2-varsigma_c_2)*f) - 1) * C
identical(varrho(0.05, "normal", 1)$c$vsxu, vsxu1) # T

vsxu2 <- ((tau_c_2^2 - (c*(c^2-varsigma_c_2)*f)^2) / (tau_c_2*(tau_c_2-c*(c^2-varsigma_c_2)*f)) - A - B) * C
identical(varrho(0.05, "normal", 2)$c$vsxu, vsxu2) # F but I think can only be numerical inaccuracy but not important
print(vsxu2, digits = 16)
print(varrho(0.05, "normal", 2)$c$vsxu, digits = 16)
