...
cu-off value $c$, i.e. for values that are larger than $1.96$ or smaller than
...
than the cut-off itself, which is why te user actually sets $\gamma_{c}$ in
...
standard errors for inference. This should work well when there are actually no
outliers but might be problematic when the sample is actually contaminated.
Since outliers might be also be re-sampled, the obtained standard errors might
be too large. This functionality is still under development.
...
sum(outlier_location %in% detected_0) # initial found 28 of 30
